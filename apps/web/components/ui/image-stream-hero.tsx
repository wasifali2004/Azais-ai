...
export type StreamImage = {
  ...
  alt?: string;
  /** Force image vs. video rendering. Auto-detected from the file extension when omitted. */
  type?: "image" | "video";
};
...
export type ImageStreamHeroProps = {
  /**
   * Media cycled onto the rails. Both rails run the same sequence, so the
        ...
        >
          {[right, left].map((name) =>
            Array.from({ length: cards }, (_, i) => {
              ...
              const media = images[i % Math.max(images.length, 1)];
              const isVideo =
                media?.type === "video" ||
                (media?.type !== "image" && VIDEO_EXT.test(media?.src ?? ""));
              return (
                <div
                  key={`${name}-${i}`}
